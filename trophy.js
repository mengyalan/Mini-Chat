...
JSJaCPacket.prototype._importNode = function (node, allChildren) {
    switch (node.nodeType) {
        case document.ELEMENT_NODE:
            ...
            if (node.attributes && node.attributes.length > 0) for (var i = 0, il = node.attributes.length; i < il; i++) {
...
JSJaCMessage.prototype.setNick = function (nick) {
    ...
    if (aNode) try {
        ...
        try {
            aNode = this.getDoc().createElementNS('http://jabber.org/protocol/nick', "nick");
        } catch (ex) {
...
function JSJaCConnection(oArg) {
    ...
    else this.setPollInterval(JSJAC_TIMERVAL);
    ...
    if (oArg && oArg.cookie_prefix) this._cookie_prefix = oArg.cookie_prefix;
...
JSJaCHttpBindingConnection.prototype._parseResponse = function (req) {
    ...
    if (body.getAttribute("type") == "terminate") {
        ...
        if (condition != "item-not-found") {
            ...
            this._connected = false;
            if (condition == "remote-stream-error") if (body.getElementsByTagName("conflict").length > 0) this._setStatus("session-terminate-conflict");
...
var LOCALES_AVAILABLE_ID = new Array('ar', 'bg', 'cs', 'de', 'eo', 'es', 'et', 'fa', 'fr', 'he', 'hu', 'id', 'it', 'ja', 'nl', 'oc', 'pl', 'pt-BR', 'ru', 'sk', 'sv', 'uk', 'zh-CN', 'zh-TW');
var LOCALES_AVAILABLE_NAMES = new Array('العربية', 'български', 'Česky', 'Deutsch', 'Esperanto', 'Español', 'Eesti keel', 'فارسی', 'Français', 'עברית', 'Magyar', 'Bahasa', 'Italiano', '日本語', 'Nederlands', 'Occitan', 'Polski', 'Brasileiro', 'Русский', 'Slovenčina', 'Svenska', 'українська', '中文简体', '中文繁體');
var XML_LANG = 'en';
var JAPPIX_STATIC = 'https://static.jappix.com/';
var JAPPIX_VERSION = 'Nemesis Alpha 1 [0.9.1]';
var JAPPIX_MAX_FILE_SIZE = '8000000';
var JAPPIX_MAX_UPLOAD = '7.63 MB';
var SERVICE_NAME = 'Jappix';
var SERVICE_DESC = 'a free social network';
var OWNER_NAME = 'FrenchTouch';
var OWNER_WEBSITE = 'http://frenchtouch.pro/';
var LEGAL = 'https://legal.jappix.com/';
var JAPPIX_RESOURCE = 'Jappix';
var LOCK_HOST = 'off';
var ANONYMOUS = 'on';
var HTTP_AUTH = null;
var REGISTRATION = 'on';
var BOSH_PROXY = 'off';
var MANAGER_LINK = 'off';
var GROUPCHATS_JOIN = '';
var ENCRYPTION = 'on';
var HTTPS_STORAGE = 'on';
var HTTPS_FORCE = 'on';
var COMPRESSION = 'on';
var MULTI_FILES = 'off';
var DEVELOPER = 'off';
var HOST_MAIN = 'jappix.com';
var HOST_MUC = 'muc.jappix.com';
var HOST_PUBSUB = 'pubsub.jappix.com';
var HOST_VJUD = 'vjud.jappix.com';
var HOST_ANONYMOUS = 'anonymous.jappix.com';
var HOST_BOSH = 'http://127.0.0.1:5280/http-bind';
var HOST_BOSH_MAIN = '/bind';
var HOST_BOSH_MINI = 'https://bind.jappix.com/';
var HOST_STATIC = 'https://static.jappix.com';
var HOST_UPLOAD = 'https://upload.jappix.com/';
...
function flushPersistent() {
    try {
        ...
        localStorage.clear();
        if (session) setPersistent('session', 1, session);
...
function generateXID(xid, type) {
    if (xid && (xid.indexOf('@') == -1)) {
        if (type == 'groupchat') return xid + '@' + HOST_MUC;
        if (xid.indexOf('.') == -1) return xid + '@' + HOST_MAIN;
        return xid;
    }
    return xid;
}
...
function strAfterLast(given_char, str) {
    if (!given_char || !str) return '';
    var char_index = str.lastIndexOf(given_char);
    var str_return = str;
    if (char_index >= 0) str_return = str.substr(char_index + 1);
    return str_return;
}
...
function allowedAnonymous() {
    if (ANONYMOUS == 'on') return true;
    return false;
}

function lockHost() {
    if (LOCK_HOST == 'on') return true;
    return false;
}
...
function relativeDate(to_parse) {
    ...
    if (current_day == old_day) return old_time;
...
function connectMini(domain, user, password) {
    try {
        oArgs = new Object();
        if (HOST_BOSH_MINI) oArgs.httpbase = HOST_BOSH_MINI;
        else oArgs.httpbase = HOST_BOSH;
        con = new JSJaCHttpBindingConnection(oArgs);
        setupConMini(con);
        var random_resource = getDB('jappix-mini', 'resource');
        if (!random_resource) random_resource = MINI_RESOURCE + ' (' + (new Date()).getTime() + ')';
        oArgs = new Object();
        oArgs.secure = true;
...
function connectedMini() {
    ...
    if (MINI_RECONNECT) logThis('Jappix Mini is now reconnected.', 3);
...
function handlePresenceMini(pr) {
    ...
    if (show == 'unavailable') {
        ...
        if (search && !nick.match(regex)) jQuery(friend).hide();
        if (is_groupchat) {
...
function notifyErrorMini() {
    jQuery('#jappix_mini').html('<div class="jm_starter">' + '<a class="jm_pane jm_button jm_images" href="' + MINI_ERROR_LINK + '" target="_blank" title="' + _e("Click here to solve the error") + '">' + '<span class="jm_counter jm_error jm_images">' + _e("Error") + '</span>' + '</a>' + '</div>');
}
...
function notifyCountersMini() {
    ...
    if (number) {
        if (jQuery('#jappix_mini div.jm_conversation:visible:first').prevAll().find('span.jm_notify').size()) jQuery('#jappix_mini a.jm_switch.jm_left').addClass('jm_notifnav');
        if (jQuery('#jappix_mini div.jm_conversation:visible:last').nextAll().find('span.jm_notify').size()) jQuery('#jappix_mini a.jm_switch.jm_right').addClass('jm_notifnav');
    }
    if (MINI_TITLE == null) return;
...
function updateOverflowMini() {
    ...
    if (number_visible <= 0) number_visible = 1;
    if (number_visible < number_total) {
        if (!jQuery('#jappix_mini a.jm_switch').size()) {
            jQuery('#jappix_mini div.jm_conversations').before('<a class="jm_switch jm_left jm_pane jm_images" href="#">' + '<span class="jm_navigation jm_images"></span>' + '</a>');
            jQuery('#jappix_mini div.jm_conversations').after('<a class="jm_switch jm_right jm_pane jm_images" href="#">' + '<span class="jm_navigation jm_images"></span>' + '</a>');
            overflowEventsMini();
        }
        var index_visible = number_visible - 1;
        jQuery('#jappix_mini div.jm_conversation:gt(' + index_visible + '):visible').hide();
        if (jQuery('#jappix_mini div.jm_conversation:hidden a.jm_pane.jm_clicked').size()) switchPaneMini();
        jQuery('#jappix_mini a.jm_switch').removeClass('jm_nonav');
        if (!jQuery('#jappix_mini div.jm_conversation:visible:first').prev().size()) jQuery('#jappix_mini a.jm_switch.jm_left').addClass('jm_nonav');
        if (!jQuery('#jappix_mini div.jm_conversation:visible:last').next().size()) jQuery('#jappix_mini a.jm_switch.jm_right').addClass('jm_nonav');
    } else {
...
function overflowEventsMini() {
    jQuery('#jappix_mini a.jm_switch').click(function () {
        if (jQuery(this).hasClass('jm_nonav')) return false;
        var hide_this = show_this = '';
        if (jQuery(this).is('.jm_left')) {
            show_this = jQuery('#jappix_mini div.jm_conversation:visible:first').prev();
            if (show_this.size()) hide_this = jQuery('#jappix_mini div.jm_conversation:visible:last');
        } else {
            show_this = jQuery('#jappix_mini div.jm_conversation:visible:last').next();
            if (show_this.size()) hide_this = jQuery('#jappix_mini div.jm_conversation:visible:first');
        }
        if (show_this && show_this.size()) {
            if (hide_this && hide_this.size()) {
                hide_this.hide();
                if (hide_this.find('a.jm_pane').hasClass('jm_clicked')) switchPaneMini();
            }
            show_this.show();
            jQuery('#jappix_mini a.jm_switch').removeClass('jm_nonav');
            if ((jQuery(this).is('.jm_right') && !show_this.next().size()) || (jQuery(this).is('.jm_left') && !show_this.prev().size())) jQuery(this).addClass('jm_nonav');
            notifyCountersMini();
        }
        return false;
    });
...
function createMini(domain, user, password) {
        ...
        typewatch(function () {
            try {
                ...
                jQuery('#jappix_mini div.jm_roster div.jm_buddies a.jm_online').each(function () {
                    ...
                    if (nick.match(regex)) jQuery(this).show();
                    else jQuery(this).hide();
                });
                jQuery('#jappix_mini div.jm_roster div.jm_grouped').each(function () {
                    if (!jQuery(this).find('a.jm_online:visible').size()) jQuery(this).hide();
                });
    ...
    jQuery(document).keydown(function (e) {
        if (jQuery('#jappix_mini div.jm_roster').is(':hidden')) return;
        if ((e.keyCode == 38) || (e.keyCode == 40)) {
            if (!jQuery('#jappix_mini a.jm_online.jm_hover').size()) {
                if (e.keyCode == 38) jQuery('#jappix_mini a.jm_online:visible:last').addClass('jm_hover');
                else jQuery('#jappix_mini a.jm_online:visible:first').addClass('jm_hover');
                ...
                if (e.keyCode == 38) hover_index--;
                else hover_index++;
                if (!hover_index) hover_index = 0;
                if (!jQuery('#jappix_mini a.jm_online:visible').eq(hover_index).size()) {
                    if (e.keyCode == 38) hover_index = jQuery('#jappix_mini a.jm_online:visible:last').index();
                    else hover_index = 0;
                }
                jQuery('#jappix_mini a.jm_friend.jm_hover').removeClass('jm_hover');
                jQuery('#jappix_mini a.jm_online:visible').eq(hover_index).addClass('jm_hover');
            }
            jQuery('#jappix_mini div.jm_roster div.jm_buddies').scrollTo(jQuery('#jappix_mini a.jm_online.jm_hover'), 0, {
                margin: true
            });
            return false;
        }
    ...
    if (suspended) {
        ...
        if (filter_value) search_box.val(filter_value).keyup();
...
function displayMessageMini(type, body, xid, nick, hash, time, stamp, message_type) {
    ...
    if ((last_xid == xid) && (message_type == last_type) && ((stamp - last_stamp) <= 1800)) grouped = true;
        ...
        if (type == 'groupchat') header += '<b class="jm_name" style="color: ' + generateColor(nick) + ';" data-xid="' + encodeQuotes(xid) + '">' + nick.htmlEnc() + '</b>';
        else if (nick == 'me') header += '<b class="jm_name jm_me" data-xid="' + encodeQuotes(xid) + '">' + _e("You") + '</b>';
        else header += '<b class="jm_name jm_him" data-xid="' + encodeQuotes(xid) + '">' + nick.htmlEnc() + '</b>';
    }
    ...
    body = body.replace(/(^|\s|>|\()((\*)([^<>'"\*]+)(\*))($|\s|<|\))/gi, '$1<b>$2</b>$6').replace(/(^|\s|>|\()((\/)([^<>'"\/]+)(\/))($|\s|<|\))/gi, '$1<em>$2</em>$6').replace(/(^|\s|>|\()((_)([^<>'"_]+)(_))($|\s|<|\))/gi, '$1<span style="text-decoration: underline;">$2</span>$6');
    ...
    else jQuery('#jappix_mini #chat-' + hash + ' div.jm_received-messages').append('<div class="jm_group jm_' + message_type + '" data-type="' + message_type + '" data-stamp="' + stamp + '">' + header + body + '</div>');
...
function switchPaneMini(element, hash) {
    ...
    if (element && (element != 'roster')) {
        ...
        if (jQuery(current).size() && jQuery(current).is(':hidden')) {
            var click_nav = '';
            if (jQuery('#jappix_mini div.jm_conversation:visible:first').prevAll().is('#' + element)) click_nav = jQuery('#jappix_mini a.jm_switch.jm_left');
            else click_nav = jQuery('#jappix_mini a.jm_switch.jm_right');
            if (click_nav) {
...
function chatMini(type, xid, nick, hash, pwd, show_pane) {
    ...
    if (!exists(current)) {
        if (type == 'groupchat') {
            if (!MINI_NICKNAME && MINI_RANDNICK) MINI_NICKNAME = randomNickMini();
        ...
        if (((type == 'groupchat') && !groupchat_exists) || ((type == 'chat') && !chat_exists) || ((type != 'groupchat') && (type != 'chat'))) html += '<a class="jm_one-action jm_close jm_images" title="' + _e("Close") + '" href="#"></a>';
        html += '</div>' + '<div class="jm_received-messages" id="received-' + hash + '"></div>' + '<form action="#" method="post">' + '<input type="text" class="jm_send-messages" name="body" autocomplete="off" placeholder="' + _e("Chat") + '" data-value="" />' + '<input type="hidden" name="xid" value="' + xid + '" />' + '<input type="hidden" name="type" value="' + type + '" />' + '</form>' + '</div>' + '<a class="jm_pane jm_chat-tab jm_images" href="#">' + '<span class="jm_name">' + nick.htmlEnc() + '</span>' + '</a>' + '</div>';
        jQuery('#jappix_mini div.jm_conversations').prepend(html);
        if (type != 'groupchat') {
            var selector = jQuery('#jappix_mini a#friend-' + hash + ' span.jm_presence');
            var show = 'available';
            if (selector.hasClass('jm_unavailable') || !selector.size()) show = 'unavailable';
    ...
    jQuery(current + ' input.jm_send-messages').focus(function () {
        ...
        if (e.keyCode == 27) {
            if (jQuery(current + ' a.jm_close').size()) {
                if (jQuery(current).next('div.jm_conversation').size()) jQuery(current).next('div.jm_conversation').find('a.jm_pane').click();
                else if (jQuery(current).prev('div.jm_conversation').size()) jQuery(current).prev('div.jm_conversation').find('a.jm_pane').click();
...
function switchChatMini() {
    if (jQuery('#jappix_mini div.jm_conversation').size() <= 1) return;
    var chat_index = jQuery('#jappix_mini div.jm_conversation:has(a.jm_pane.jm_clicked)').index();
    chat_index++;
    if (!chat_index) chat_index = 0;
    if (!jQuery('#jappix_mini div.jm_conversation').eq(chat_index).size()) chat_index = 0;
    ...
    if (chat_hash) switchPaneMini('chat-' + chat_hash, chat_hash);
}
...
function initializeMini() {
    ...
    for (var j = 0; j < MINI_CHATS.length; j++) {
        ...
        try {
            ...
            if (!chat_nick) chat_nick = getXIDNick(chat_xid);
            else chat_nick = unescape(chat_nick);
            chatMini('chat', chat_xid, chat_nick, chat_hash);
        } catch (e) {}
    }
    if (!MINI_AUTOCONNECT && !MINI_GROUPCHATS.length && !MINI_CHATS.length) jQuery(document).oneTime(10, function () {
        showRosterMini();
    });
}
...
function addBuddyMini(xid, hash, nick, groupchat, subscription) {
    ...
    var code = '<a class="jm_friend jm_offline" id="friend-' + hash + '" data-xid="' + escape(xid) + '" data-nick="' + escape(nick) + '" data-hash="' + hash + '" href="#" ' + substr + '><span class="jm_presence jm_images jm_unavailable"></span>' + nick.htmlEnc() + '</a>';
...
function handleRosterMini(iq) {
    ...
    for (var i = 0; i < x; i++) {
        ...
        else addBuddyMini(xid, hash, nick, null, subscription);
    }
...
function randomNickMini() {
    var first_arr = ['Illini-'];
    var second_arr = ['io', 'ice', 'a', 'u', 'o', 'ou', 'oi', 'ana', 'oro', 'izi', 'ozo', 'aza', 'ato', 'ito', 'ofa', 'oki', 'ima', 'omi'];
    var last_arr = ['t', 'z', 'r', 'n', 'tt', 'zz', 'pp', 'j', 'k', 'v', 'c', 'x', 'ti', 'to', 'ta', 'ra', 'ro', 'ri'];
...
function launchMini(autoconnect, show_pane, domain, user, password) {
    ...
    else MINI_ANONYMOUS = false;
    if (autoconnect && hasDB()) MINI_AUTOCONNECT = true;
    else MINI_AUTOCONNECT = false;
    if (show_pane) MINI_SHOWPANE = true;
    ...
    if ((BrowserDetect.browser == 'Explorer') && (BrowserDetect.version < 7)) jQuery('head').append('<link rel="stylesheet" href="' + JAPPIX_STATIC + 'php/get.php?t=css&amp;f=mini-ie.css" type="text/css" media="all" />');
    jQuery(document).keydown(function (e) {
